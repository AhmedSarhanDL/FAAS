...
if [ -f "main.pdf" ]; then
    ...
    # Try with lualatex as a fallback
    echo "Trying with lualatex as a fallback..."
    docker run --rm -v "$CURRENT_DIR:/workdir" $DOCKER_IMAGE sh -c "cd /workdir && \
    ...
    if [ -f "main.pdf" ]; then
        mv "main.pdf" "output/el_tor_circular_economy_en.pdf"
        echo "✓ Successfully created output/el_tor_circular_economy_en.pdf using lualatex"
    else
        echo "✗ Failed to compile main.tex with lualatex as well"
    fi
fi
